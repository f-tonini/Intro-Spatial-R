...
setwd("~/Documents/Intro-Spatial-R/module3/")	
#' 	
...
#' 	
#' 	
all.equal(proj4string(deer_unit), proj4string(utah.wgs84))
all.equal(proj4string(utah.wgs84), proj4string(ndvi500m.wgs84))	
#' 	
#' The `all.equal` function returns `TRUE` if the objects are all the same, and `FALSE` otherwise. So, all of these layers have exactly the same character `proj4` string. 	
#' 	
...
system.time(deer_unit.rast <- rasterize(deer_unit, ndvi_utah, deer_unit$UNum)) # note this took ~5-7 seconds on my 16 GB RAM laptop	
deer_unit.rast	
...
# Produced a warning that the field (attribute) names were abbreviated. You will want to check to see that they make sense in the new file.
